...
import { TemplateCoreServicesModule } from '../../services';
...
export class ListPageComponent extends BaseListPage implements AfterViewInit {
  ...
  private readonly templateAppStateService = inject(ITemplateAppStateService);
  ...
  constructor() {
    const params = inject(TemplateComponentBaseParams);

    super(params);
    const templateAppStateService = this.templateAppStateService;
    this.preloader.markAsPreloaded('list');
    if (location.pathname.includes('/lists')) {
      // TODO: document why & how it is possible
      return;
    }
    templateAppStateService.changed.subscribe((appState) => {
  ...
  public clickShowWatchedMovies(): void {
    this.templateAppStateService.setShowWatched(this.isHideWatched());
  }
  ...
  // 	if (this.appService.appId === 'template') {
